...
const BookRoomCTA:FC<Props> = props => {
    ...
    const {price, discount, specialNote, checkinDate, setCheckinDate, checkoutDate, setCheckoutDate, calcMinCheckoutDate,
    setAdults, setNoOfChildren, adults, noOfChildren, isBooked, handleBookNowClick,}=props;
    ...
    return <div className="px-7 py-6">
        <h3>
            ...
            {discount ? (<span className="font-bold text-xl">
                {' '} | Discount {discount}%. Now {' '} <span className="text-tertiary-dark">₹ {discountPrice}</span> </span>
                ):(
                    ''
                )
            }
        ...
        <div className="flex ">
            <div className="w-1/2 pr-2">
                ...
                dateFormat='dd/MM/yyyy' minDate={new Date()} id="check-in-date" className="w-full border text-black border-gray-300 rounded-lg p-2.5 focus:ring-primary focus:border-primary" />
            </div>
            <div className="w-1/2 pl-2">
                ...
                dateFormat='dd/MM/yyyy' disabled={!checkinDate} minDate={calcMinCheckoutDate()} id="check-out-date" className="w-full border text-black border-gray-300 rounded-lg p-2.5 focus:ring-primary focus:border-primary" />
            </div>
        ...
        {calcNoOfDays() > 0? <p className="mt-2"> 
